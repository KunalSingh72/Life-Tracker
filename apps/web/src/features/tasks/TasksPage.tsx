...
export default function TasksPage() {
  ...
  return (
    <div className="flex flex-col h-full absolute inset-0 z-20 bg-background-surface md:static md:bg-transparent md:max-w-4xl md:mx-auto">
      {/* Mobile-Only Header */}
      <div className="md:hidden flex items-center justify-between px-4 h-16 border-b border-border-subtle shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-2xl font-extrabold bg-gradient-to-r from-text-primary via-accent-primary to-text-primary bg-clip-text text-transparent">
            TODO
          </span>
        </div>
      </div>
      ...
      <div className="hidden md:flex py-8 flex-col items-center shrink-0">
        <h1 className="text-5xl font-extrabold tracking-tight mb-8">
          <span className="bg-gradient-to-r from-text-primary via-accent-primary to-text-primary bg-clip-text text-transparent bg-[length:200%_auto] animate-gradient">
            TODO
          </span>
        </h1>
      ...
      <div className="flex-1 overflow-hidden flex flex-col md:bg-background-surface md:border border-border-subtle md:rounded-2xl md:shadow-sm md:mb-8">
        {/* Container Actions (Now visible on both mobile and desktop) */}
              ...
              >
                <div className="flex items-center gap-3 md:gap-4">
                  <button
                    ...
                    className="p-2 -ml-2 flex-shrink-0 cursor-pointer"
                  >
                    {task.completed ? (
                  ...
                  <div className="flex flex-col">
                    <span
                      className={`font-medium transition-all ${task.completed ? "text-text-secondary line-through" : "text-text-primary"}`}
                    >
                      {task.title}
                    </span>
                ...
                <div className="relative">
                  ...
                  {activeDropdown === task.id && (
                    ...
                    >
                      ...
                      <button
                        onClick={(e) => updatePriority(task.id, "medium", e)}
                        className="w-full text-left px-4 py-3 md:py-2 text-sm text-text-primary hover:bg-background-main flex items-center gap-2 cursor-pointer"
                      >
                        <Flag
